...
interface CustomTShirtModalProps {
  isOpen: boolean
  onClose: () => void
}

export default function CustomTShirtModal({
  isOpen,
  onClose,
}: CustomTShirtModalProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data?.products && data.products.length > 0) {
          // Filter to only show shirt products (BASIC T-SHIRTS, PREMIUM TEES, NO MINIMUMS)
          const shirtProducts = data.products.filter((product: Product) => {
            const title = product.title.toUpperCase()
            return (
              title.includes("BASIC T-SHIRT") ||
              title.includes("PREMIUM TEE") ||
              title.includes("NO MINIMUM")
            )
          })
      ...
      >
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-5xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-2xl font-bold">Custom T-Shirts</h2>
            <Button
              variant="ghost"
              size="icon"
              onClick={handleClose}
