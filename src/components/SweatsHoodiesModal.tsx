...
interface SweatsHoodiesModalProps {
  isOpen: boolean
  onClose: () => void
}

export default function SweatsHoodiesModal({
  isOpen,
  onClose,
}: SweatsHoodiesModalProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data?.products && data.products.length > 0) {
          // Filter to only show hoodies and crewnecks
          const sweatsHoodiesProducts = data.products.filter((product: Product) => {
          ...
          setProducts(sweatsHoodiesProducts)
        }
      }
    } catch (error) {
      ...
      >
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-5xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-2xl font-bold">Sweats & Hoodies</h2>
            <Button
              variant="ghost"
              size="icon"
              onClick={handleClose}
